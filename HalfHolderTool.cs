...
using Valve.VR;
public class HalfHolderTool : MonoBehaviour {
    ...
    void Update()
    {
        if (Input.GetKeyDown("h") || SteamVR_Actions.default_Teleport.GetStateDown(SteamVR_Input_Sources.Any))
        {
    ...
    private void Cut()
    {
        ...
        Plane splitPlaneLeftUp = new Plane(Vector3.Normalize(transform.TransformVector(normals[0])), transform.TransformPoint(vertices[0]) + modelThickness * Vector3.Normalize(transform.TransformVector(normals[0])));
        Debug.Log("splitPlaneLeftUp:" + splitPlaneLeftUp);
        Plane splitPlaneRightUp = new Plane(-Vector3.Normalize(transform.TransformVector(normals[0])), transform.TransformPoint(vertices[6]) - modelThickness * Vector3.Normalize(transform.TransformVector(normals[0])));
        Plane splitPlaneMiddle = new Plane(Vector3.Normalize(transform.TransformVector(normals[0])), (transform.TransformPoint(vertices[0]) + transform.TransformPoint(vertices[6])) / 2);
        abovePlaneOfOsteotome = new Plane(beCutThing.transform.InverseTransformDirection(Vector3.Normalize(transform.TransformVector(normals[4]))), beCutThing.transform.InverseTransformPoint(transform.TransformPoint(vertices[4])));  //计算出相对被切割物体的坐标的平面

        underPlaneOfOsteotome = new Plane(beCutThing.transform.InverseTransformDirection(Vector3.Normalize(transform.TransformVector(normals[4]))), beCutThing.transform.InverseTransformPoint(transform.TransformPoint(vertices[7])));//计算出相对被切割物体的坐标的平面
        ...
        forwardPlaneOfOsteotome = new Plane(-beCutThing.transform.InverseTransformDirection(fb), beCutThing.transform.InverseTransformPoint(transform.TransformPoint(vertices[0]))); //计算出相对被切割物体的坐标的平面
        backPlaneOfOsteotome = new Plane(beCutThing.transform.InverseTransformDirection(fb), beCutThing.transform.InverseTransformPoint(transform.TransformPoint(vertices[1])));//计算出相对被切割物体的坐标的平面
        outsideForwardPlaneOfOsteotome = forwardPlaneOfOsteotome;
        outsideBackPlaneOfOsteotome = backPlaneOfOsteotome;
    ...
    private void GenerateShell(out GameObject shell, out ArrayList triDown, out ArrayList triUp)
    {
        ...
        triUp = new ArrayList();
        shell = new GameObject("HaftHolder");
