...
using Valve.VR;
//手术导板
public class HolderTool : MonoBehaviour {
    ...
    private void HandHoverUpdate(Hand hand)
    {
        ...
        if (g!=null)//说明正在切面上转动
        {
            ...
            //g.transform.LookAt(cutPlane.ClosestPointOnPlane(transform.position));
            Quaternion NextRot = Quaternion.LookRotation(cutPlane.normal, Vector3.Cross(cutPlane.normal, cutPlane.ClosestPointOnPlane(transform.position) - g.transform.position));
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(1) || SteamVR_Actions.default_Teleport.GetStateDown(SteamVR_Input_Sources.Any))
        {
            ...
            if (status == 0)//在CutPosition里面找到最近的切割面，把导板工具固定在那个面上
            {
                int n = CutPosition.cutObject.Count;
                if (n > 0)//说明目前已经有物体被切割，可以生成导板,找到离本导板工具最近的切割面
                {
                    status = 1;
                    int index = 0;
                    float distance;
                    ...
                    for (int i = 0;i<n;i++)
                    {
                        distance = (transform.position - CutPosition.cutObject[i].transform.TransformPoint(CutPosition.cutPoint[i])).magnitude;//定死
                    ...
                    beCutThing = CutPosition.cutObject[index];
                    transform.position = point;
                    //Vector3 forward = cutPlane.ClosestPointOnPlane(transform.position) - point;
    ...
    private void Cut()
    {
        ...
        Vector3[] vertices = transform.GetComponent<MeshFilter>().mesh.vertices;
        Vector3[] normals = transform.GetComponent<MeshFilter>().mesh.normals;
        Plane splitPlaneLeftDown = new Plane(transform.TransformPoint(vertices[0]), transform.TransformPoint(vertices[2]), transform.TransformPoint(vertices[1]));
        Debug.Log("splitPlaneLeftDown: " + splitPlaneLeftDown);
        Plane splitPlaneRightDown = new Plane(-Vector3.Normalize(transform.TransformVector(normals[0])), transform.TransformPoint(vertices[6]));
        Plane splitPlaneLeftUp = new Plane(Vector3.Normalize(transform.TransformVector(normals[0])), 
            transform.TransformPoint(vertices[0]) + modelThickness * Vector3.Normalize(transform.TransformVector(normals[0])));

        Debug.Log("splitPlaneLeftUp:" + splitPlaneLeftUp);
        Plane splitPlaneRightUp = new Plane(-Vector3.Normalize(transform.TransformVector(normals[0])), 
            transform.TransformPoint(vertices[6]) - modelThickness * Vector3.Normalize(transform.TransformVector(normals[0])));

        Plane splitPlaneMiddle = new Plane(Vector3.Normalize(transform.TransformVector(normals[0])), 
            (transform.TransformPoint(vertices[0]) + transform.TransformPoint(vertices[6])) / 2);

        abovePlaneOfOsteotome = new Plane(beCutThing.transform.InverseTransformDirection(Vector3.Normalize(transform.TransformVector(normals[4]))), 
            beCutThing.transform.InverseTransformPoint(transform.TransformPoint(vertices[4])));  //计算出相对被切割物体的坐标的平面

        underPlaneOfOsteotome = new Plane(beCutThing.transform.InverseTransformDirection(Vector3.Normalize(transform.TransformVector(normals[4]))), 
            beCutThing.transform.InverseTransformPoint(transform.TransformPoint(vertices[7])));//计算出相对被切割物体的坐标的平面

        Vector3 fb = Vector3.Cross(Vector3.Normalize(transform.TransformVector(normals[0])), Vector3.Normalize(transform.TransformVector(normals[4])));
        forwardPlaneOfOsteotome = new Plane(-beCutThing.transform.InverseTransformDirection(fb), 
            beCutThing.transform.InverseTransformPoint(transform.TransformPoint(vertices[0]))); //计算出相对被切割物体的坐标的平面

        backPlaneOfOsteotome = new Plane(beCutThing.transform.InverseTransformDirection(fb), 
            beCutThing.transform.InverseTransformPoint(transform.TransformPoint(vertices[1])));//计算出相对被切割物体的坐标的平面

        outsideForwardPlaneOfOsteotome = new Plane(forwardPlaneOfOsteotome.normal, 
            forwardPlaneOfOsteotome.ClosestPointOnPlane(Vector3.zero) 
            + modelThickness / beCutThing.transform.lossyScale.x * Vector3.Normalize(forwardPlaneOfOsteotome.normal));//取点时增加一个厚度

        outsideBackPlaneOfOsteotome = new Plane(backPlaneOfOsteotome.normal, backPlaneOfOsteotome.ClosestPointOnPlane(Vector3.zero) 
            + modelThickness / beCutThing.transform.lossyScale.x * Vector3.Normalize(backPlaneOfOsteotome.normal));

        cutVector = Vector3.Normalize(transform.TransformVector(vertices[0]) - transform.TransformVector(vertices[2]));
        //Debug.Log("cutVector:" + cutVector);
        ///*
        //3.根据切面提取被切割物体的一定范围表面，并构造成膜
        GameObject shell;
        ArrayList triDown, triUpAndOther; //顺便分别记录膜上下表面的三角面，为下一步作准备
        GenerateShell(out shell, out triDown, out triUpAndOther);//有问题 2020.08.03

        //2020.08.11 尝试做一个mesh分离-->不会分离,仅添加切割面
        //Isolate.IsolateMesh(shell);
       ///*
        ...
        ArrayList angularPoints1 = MeshChange(splitPlaneLeftDown, splitPlaneRightDown, mesh);      //关键位置                                        //MeshChange   用两个面切膜的下层表面

        ///*
        ArrayList triangle1 = new ArrayList(mesh.triangles);//把经过MeshChange处理过的下层表面三角面保存到triangle1
        mesh.triangles = (int[])triUpAndOther.ToArray(typeof(int));   //替换mesh的三角面为膜上层表面和缝合面
        inner = false;
        ArrayList angularPoints2 = MeshChange(splitPlaneLeftUp, splitPlaneRightUp, mesh);     //关键位置                                                  //MeshChange   用两个面分别切膜的上层表面
        ArrayList triangle2 = new ArrayList(mesh.triangles);
        ...
        triangle1.Add((int)angularPointsOfDownLeft[0]);
        triangle1.Add((int)angularPointsOfUpLeft[0]);
        triangle1.Add((int)angularPointsOfDownLeft[1]);
        triangle1.Add((int)angularPointsOfUpLeft[0]);
        triangle1.Add((int)angularPointsOfDownLeft[0]);
        triangle1.Add((int)angularPointsOfUpLeft[1]);

        triangle1.Add((int)angularPointsOfDownLeft[3]);
        triangle1.Add((int)angularPointsOfDownLeft[2]);
        triangle1.Add((int)angularPointsOfUpLeft[3]);
        triangle1.Add((int)angularPointsOfDownLeft[3]);
        triangle1.Add((int)angularPointsOfUpLeft[3]);
        triangle1.Add((int)angularPointsOfUpLeft[2]);

        //////
        triangle1.Add((int)angularPointsOfDownRight[0]);
        triangle1.Add((int)angularPointsOfUpRight[0]);
        triangle1.Add((int)angularPointsOfDownRight[1]);
        triangle1.Add((int)angularPointsOfUpRight[0]);
        triangle1.Add((int)angularPointsOfDownRight[0]);
        triangle1.Add((int)angularPointsOfUpRight[1]);

        triangle1.Add((int)angularPointsOfDownRight[3]);
        triangle1.Add((int)angularPointsOfDownRight[2]);
        triangle1.Add((int)angularPointsOfUpRight[3]);
        triangle1.Add((int)angularPointsOfDownRight[3]);
        triangle1.Add((int)angularPointsOfUpRight[3]);
        triangle1.Add((int)angularPointsOfUpRight[2]);
        /////
        triangle1.Add((int)angularPointsOfDownLeft[1]);
        triangle1.Add((int)angularPointsOfUpLeft[0]);
        triangle1.Add((int)angularPointsOfUpRight[1]);
    ...
    private void GenerateShell( out GameObject shell, out ArrayList triDown, out ArrayList triUp)
    {
        ...
        for (int i = 0; i < vertices.Length; i++)
        {
            verticesInWorldSpace[i] = beCutThing.transform.TransformPoint(vertices[i]);   //把顶点用世界坐标表示,再去提取distance范围的顶点(由于不同物体的scale不一样,这样做distance的值才方便设置) 、根据不同物体scale去修正distance或许可行
        }
        ...
        //构造外层表面 -->复制&&延展&&微调
        normals = nor.ToArray();
        vertices = ver.ToArray();
        triangles = tri.ToArray();//保存数据
        int n = ver.Count;
        for (int i = 0; i < n; i++)
        {
            ver.Add(ver[i] + modelThickness / beCutThing.transform.lossyScale.x * normals[i]);
            //Debug.Log("ver[n-1+i]==ver[i]:" + (ver[n - 1 + i] == ver[i]));
            nor.Add(nor[i]);
        }
        //--------------------------------------------------------------------------------------------------
        //为了避免尖锐边角顶点分离，但由于复杂度高可能会无法在大模型上运行                     可用哈希表改进
        ...
        for (int i = 0; i < vertices.Length; i++)  //寻找重复点,放入数组
        {
            ...
            {
                if (vertices[i] == vertices[j])//？ 加完这次跳出循环如何 可避免加入两次重复点 2020.8.3
                {
                    repetivePoint.Add(j);
        ...
        {
            tri.Add((int)cutPointIndex[i * 2]);
            tri.Add((int)cutPointIndex[i * 2 + 1]);
       ...
       // */
        ...
        //shell.GetComponent<Rigidbody>().drag = 11111111111;
        //shell.GetComponent<Rigidbody>().angularDrag = 11111111111;
    ...
    protected ArrayList MeshChange(Plane splitPlaneLeft, Plane splitPlaneRight, Mesh theMesh)
    {
        ...
        ArrayList cutPointRightDown = (ArrayList)cutPoints2[1];//tip 夹板区域四个顶点
    ...
    //    //if()
    //    Gizmos.DrawSphere(transform.GetComponent<MeshFilter>().mesh.vertices[0], 0.005f);
    //    Gizmos.DrawSphere(transform.GetComponent<MeshFilter>().mesh.vertices[2], 0.005f);
    //    Debug.Log("=====打印完成=====");
    //}
}
