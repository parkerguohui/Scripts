...
/// 实时切割
/// </summary>
public class Realtime_Cut : MonoBehaviour {
    ...
    private void Cut()
    {
        ...
        {
            Debug.Log("第" + i + "边:" + cutEdges1[2 * i].x + " " + cutEdges1[2 * i].y + " " + cutEdges1[2 * i].z + " " + cutEdges1[2 * i + 1].x + " " + cutEdges1[2 * i + 1].y + " " + cutEdges1[2 * i + 1].z);
        ...
        */
        ...
        //修改版
        //缝合第一个面
        int n = cutEdges1.Count / 2;
        List<bool> visit = new List<bool>(n);
        for (int i = 0; i < n; i++)
            visit.Add(false);   
        ...
        List<int> outline = new List<int>(2 * n);
        while (visitTime != n)
        {
            //找到新头部、尾部
            if (head != 0 || tail != 0)
            ...
            Debug.Log("开始~");
            FindFromTail(cutEdges1, ref visit, head, ref tail, ref points, ref outline, ref visitTime);
            Debug.Log("F1");
            FindFromHead(cutEdges1, ref visit, ref head, tail, ref points, ref outline, ref visitTime);
            Debug.Log("F2");
            outline.Add(tail);
            outline.Add(head);
        }
        ...
        outline = new List<int>(2 * n);
        visit = new List<bool>(n);
        for (int i = 0; i < n; i++)
            visit.Add(false);
        while (visitTime != n)
        {
            FindFromTail(cutEdges2, ref visit, head, ref tail, ref points, ref outline, ref visitTime);
            FindFromHead(cutEdges2, ref visit, ref head, tail, ref points, ref outline, ref visitTime);
            outline.Add(tail);
            outline.Add(head);
        }
        //Debug.Log("1");
